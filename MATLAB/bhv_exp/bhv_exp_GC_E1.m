...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
res_fold=['bhv_results_E1/', subjid, '/'];
...
try 
   ...
   %used test_keys script to find key ids
    ...
    fl=['stims_ord_E1/', subjid, '_ord_mat.txt'];
    ...
    for run_number=1:5
        ...
        for trial_k=1:240
                ...
                %stim=imresize(stim, 0.75, 'nearest');
                Screen('PutImage', w, stim);
                                                    %trialStartTime_ckeck=sprintf('%08.4f', trialStartTime)
                                                    %GetSecs_check=sprintf('%08.4f', GetSecs)
                                                    %stimStartTime_check=sprintf('%08.4f', stimStartTime)

                WaitSecs('UntilTime', stimStartTime);        
                Screen('Flip', w);        
                ...
                %%% button press or stim display expires
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                                    %GetSecs_check=sprintf('%08.4f', GetSecs)
                                                    %stimEndTime_max_check=sprintf('%08.4f', stimEndTime_max)
                ...
                if ~keyCodes(g_key)
                    ...
                    if sum(double(keyCodes([key1 key2])))>0
                        ...
                        acc=(trial_gend_code==1 & resp==1) | (trial_gend_code==2 & resp==2)
                        RT_act=RT;
                    else resp=NaN; acc=NaN; RT_act=NaN;
                    end

                    res_vect=[lr_ind(trial_k) trial_id_code ord_gend_vect(trial_k, 1) resp acc RT_act  ]
                    
                    dlmwrite(resfile_nm, res_vect, '-append') % 'precision', '%05.2f'           
                    fprintf(fid,'%02.3f %02.3f %02.3f \n', stimStartTime_rec, stimEndTime_rec, RT);
        ...
        if trial_k~=240 %if debugging, use some random test values
            acc=0.52; m_acc=0.91; f_acc=0.8; RT_mn=2.2; consist=0.28;
        else 
            [acc, m_acc, f_acc, RT_mn, consist]=anl_acc_consist_GC_E1({resfile_nm})
        end

        fprintf(fid,'%s \n','<><><><><><><><>');
        fprintf(fid,'%s %02.2f %s %02.2f %s %02.2f \n','Overall acc: ', acc, '   Male acc: ', m_acc, '   Fem acc: ', f_acc);
        fprintf(fid,'%s %02.2f \n','RT (mean): ', RT_mn);
        fprintf(fid,'%s %02.2f \n','Consist: ', consist);
        fprintf(fid,'%s \n','<><><><><><><><>');
        fprintf(fid,'%s %s \n','End:', datestr(now));         
        ...
        local_doEndScreen(w, black, run_number, esc_key, acc*100, RT_mn, consist)
...
switch instNum
        case {1}
                ...
                Screen('DrawText', window, 'Your task is to decide whether a face is male or female.', inst_horzpos, top_vertpos+inst_vertpos*2, inst_color);
                Screen('DrawText', window, 'If male, press the 3 key with your left index finger.', inst_horzpos, top_vertpos+inst_vertpos*3, inst_color);
                Screen('DrawText', window, 'If female, press the 0 key with your right index finger.', inst_horzpos, top_vertpos+inst_vertpos*4, inst_color);
                Screen('DrawText', window, 'Please, try to be as accurate as possible!', inst_horzpos, top_vertpos+inst_vertpos*5, inst_color);
                Screen('DrawText', window, 'Also, try to respond as quickly as possible (and always before the fixation cross dims)!', inst_horzpos, top_vertpos+inst_vertpos*6, inst_color);              
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function local_doEndScreen(window, bkgdColor, run_number, esc_key, acc, RT_mn, consist)%inputDev, , enter_key
...
Screen('DrawText', window, ['End of run ', num2str(run_number), ' (out of 5).'], inst_horzpos, top_vertpos+inst_vertpos, inst_color);
    
acc_txt=sprintf('Accuracy is  %02.2f%s', acc, '%')
Screen('DrawText', window, acc_txt, inst_horzpos, top_vertpos+inst_vertpos*3);
