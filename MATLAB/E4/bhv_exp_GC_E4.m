...
res_fold=['bhv_results_E4/', subjid, '/'];
...
%% Display and image loading procedures   
    ...
    fl=['stims_ord_E4/', subjid, '_ord_mat.txt'];
...
%% Main Loop %%
    ...
    %}
        ...
        if trial_k~=295 %if debugging, use some random test values
            ...
            [RT_mn, consist, min, maxm, total_rightvote, total_leftvote, self_consist]=anl_acc_consist_GC_E4({resfile_nm}, subjid, run_number);
        end
...
switch instNum
        case {1}
                Screen('DrawText', window, 'In this experiment you will be presented with pairs of female faces.', inst_horzpos, top_vertpos+inst_vertpos, inst_color); 
                Screen('DrawText', window, 'Your task is to select the more feminine face from each pair.', inst_horzpos, top_vertpos+inst_vertpos*2, inst_color);
                Screen('DrawText', window, 'If the left face is more feminine, press the 3 key.', inst_horzpos, top_vertpos+inst_vertpos*3, inst_color);
                Screen('DrawText', window, 'If the right face is more feminine, press the 0 key.', inst_horzpos, top_vertpos+inst_vertpos*4, inst_color);
...
elseif call_exp==2
    Screen('DrawText', window, 'Take a short break if needed.  Please call the experimenter to continue!', inst_horzpos, top_vertpos+inst_vertpos*10);
elseif call_exp==3
    Screen('DrawText', window, 'You ranked these images as being least feminine and most feminine during this run.', inst_horzpos, top_vertpos+inst_vertpos*5, inst_color);
    min_img=imread(strcat('stims/stim20', sprintf('%02d', min), '1.tif'));
    maxm_img=imread(strcat('stims/stim20', sprintf('%02d', maxm), '1.tif'));
